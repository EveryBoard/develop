...
var ResetPasswordComponent = class _ResetPasswordComponent {
  ...
  success = false;
  errorMessage = import_lib.MGPOptional.empty();
  email = "";
  resetPassword() {
    return __async(this, null, function* () {
      import_lib.Utils.assert(this.email !== "", "No email was entered, but it should not be possible to submit the form then!");
      this.errorMessage = import_lib.MGPOptional.empty();
      this.success = false;
      const result = yield this.connectedUserService.sendPasswordResetEmail(this.email);
      if (result.isSuccess()) {
        this.success = true;
      } else {
        this.errorMessage = import_lib.MGPOptional.of(result.getReason());
      }
    });
  }
...
//# sourceMappingURL=reset-password.component-UQVZAGTZ.js.map
