...
var RulesConfigUtils = class {
  ...
  static getGameDefaultConfig(gameName) {
    const gameInfo = GameInfo.getByUrlName(gameName);
    Utils.assert(gameInfo.isPresent(), `Game does not exist but it should: ${gameName}`);
    return gameInfo.get().getRulesConfig();
  }
};
...
var GameWrapper = class _GameWrapper extends BaseWrapperComponent {
  ...
  createGameComponent(component) {
    return __async(this, null, function* () {
      ...
      this.gameComponent.setClickInterceptor((fn, clickNamer) => (...args) => __async(this, null, function* () {
        const clickedElementName = clickNamer(...args);
        const clickValidity = yield this.gameComponent.canUserPlay(clickedElementName);
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(GameWrapper, { className: "GameWrapper", filePath: "src/app/components/wrapper-components/GameWrapper.ts", lineNumber: 32 });
})();
...
//# sourceMappingURL=chunk-LCO3Y5NP.js.map
