...
var TutorialGameWrapperComponent = class TutorialGameWrapperComponent2 extends GameWrapper {
  ...
  currentReason = import_lib.MGPOptional.empty();
  moveAttemptMade = false;
  stepFinished = [];
  tutorialOver = false;
  canUserPlay(elementName) {
    return __async(this, null, function* () {
      this.currentReason = import_lib.MGPOptional.empty();
      if (this.stepFinished[this.stepIndex] || this.moveAttemptMade) {
        return import_lib.MGPValidation.failure(TutorialFailure.STEP_FINISHED());
      }
      const currentStep = this.steps[this.stepIndex];
      if (currentStep.isClick()) {
        this.gameComponent.hideLastMove();
        this.moveAttemptMade = true;
        if (import_lib.Utils.getNonNullable(currentStep.acceptedClicks).some((m) => m === elementName)) {
        ...
        return import_lib.MGPValidation.failure(TutorialFailure.INFORMATIONAL_STEP());
      }
    });
  }
  onCancelMove(reason) {
    return __async(this, null, function* () {
      ...
      if (reason !== void 0) {
        this.currentReason = import_lib.MGPOptional.of(reason);
      }
  ...
  onLegalUserMove(move) {
    return __async(this, null, function* () {
      ...
      const node = this.gameComponent.rules.choose(this.gameComponent.node, move, config);
      import_lib.Utils.assert(node.isSuccess(), "It should be impossible to call onLegalUserMove with an illegal move, but got " + node.getReasonOr(""));
      ...
      if (currentStep.isPredicate()) {
        ...
        const moveValidity = import_lib.Utils.getNonNullable(currentStep.predicate)(move, previousState, resultingState);
        if (moveValidity.isSuccess()) {
          this.showStepSuccess(currentStep.getSuccessMessage());
        } else {
          this.currentReason = import_lib.MGPOptional.of(moveValidity.getReason());
        }
        ...
        if (currentStepMove.acceptedMoves.some((m) => m.equals(move))) {
          ...
          this.currentReason = import_lib.MGPOptional.of(currentStepMove.getFailureMessage());
        }
      } else {
        import_lib.Utils.assert(currentStep.isClick(), "Here, we should have a click");
      }
  ...
  getCurrentStepTitle() {
    import_lib.Utils.assert(this.steps.length > 0, "Tutorial has no step");
  ...
  showSolution() {
    return __async(this, null, function* () {
      ...
      import_lib.Utils.assert(step.hasSolution(), "step must have solution");
...
//# sourceMappingURL=chunk-5CIFRPWI.js.map
