...
import "./chunk-BLR7XPE6.js";
import "./chunk-3SNUTSIQ.js";
...
var LocalGameWrapperComponent = (_a = class extends GameWrapper {
  ...
  winnerMessage = import_lib.MGPOptional.empty();
  rulesConfig;
  // Set in constructor and in ngAfterViewInit
  constructor() {
    super();
    this.players = [import_lib.MGPOptional.of(this.playerSelection[0]), import_lib.MGPOptional.of(this.playerSelection[1])];
      ...
      if (noConfigIsProvided || gameIsNotConfigurable) {
        ...
        for (const key of rulesConfigDescription.getFields()) {
          ...
          if (paramValue == null) {
            ...
            const value = import_lib.JSONParser.parseJSONSafely(paramValue);
            if (value.isPresent()) {
              const actualValue = value.get();
              if ((0, import_lib.isJSONPrimitive)(actualValue) && rulesConfigDescription.isValid(key, actualValue)) {
        ...
        this.rulesConfig = import_lib.MGPOptional.of(config);
      }
    });
  }
  ...
  updatePlayer(player) {
    return __async(this, null, function* () {
      this.players[player.getValue()] = import_lib.MGPOptional.of(this.playerSelection[player.getValue()]);
        ...
        if (gameStatus.winner.isPlayer()) {
          ...
          if (this.players[gameStatus.winner.getValue()].equalsValue("human")) {
            if (this.players[loserValue].equalsValue("human")) {
              this.winnerMessage = import_lib.MGPOptional.of($localize`${winner} won`);
            } else {
              this.winnerMessage = import_lib.MGPOptional.of($localize`You won`);
            }
          } else {
            if (this.players[loserValue].equalsValue("human")) {
              this.winnerMessage = import_lib.MGPOptional.of($localize`You lost`);
            } else {
              this.winnerMessage = import_lib.MGPOptional.of($localize`${this.players[gameStatus.winner.getValue()].get()} (Player ${gameStatus.winner.getValue() + 1}) won`);
            }
          }
        }
      }
  ...
  getPlayingAI() {
    ...
    if (aiOpt.isPresent()) {
      ...
      const matchingOptions = import_lib.MGPOptional.ofNullable(ai.availableOptions.find((options) => {
   ...
   */
  getAI(playerIndex) {
    return import_lib.MGPOptional.ofNullable(this.gameComponent.availableAIs.find((a) => {
  ...
  doAIMove(playingAI, options) {
    return __async(this, null, function* () {
      ...
      import_lib.Utils.assert(gameStatus === GameStatus.ONGOING, "AI should not try to play when game is over!");
      ...
      if (nextNode.isSuccess()) {
        ...
        yield this.applyNewMove();
        return import_lib.MGPValidation.SUCCESS;
      } else {
  ...
  handleAIError(playingAI, illegalMove, error) {
    return __async(this, null, function* () {
      ...
      return import_lib.Utils.logError("LocalGameWrapper", "AI chose illegal move", {
  ...
  takeBack() {
    return __async(this, null, function* () {
      ...
      if (this.isTurnOfPlayingAI()) {
        import_lib.Utils.assert(this.gameComponent.node.parent.isPresent(), "Cannot take back in first turn when AI is Player.ZERO");
        this.gameComponent.node = this.gameComponent.node.parent.get();
      }
  ...
  restartGame() {
    return __async(this, null, function* () {
      ...
      this.endGame = false;
      this.winnerMessage = import_lib.MGPOptional.empty();
      yield this.proposeAIToPlay();
    });
  }
  getPlayer() {
...
//# sourceMappingURL=local-game-wrapper.component-OTRRH5WP.js.map
