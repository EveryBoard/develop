...
var Debug = class _Debug {
  ...
  static isVerbose(name) {
    ...
    try {
      ...
      if (verbosity[name] == null)
        return [false, false];
      import_lib.Utils.assert(Array.isArray(verbosity[name]), `malformed verbosity levels for ${name}: ${verbosity[name]}`);
  ...
  static log(constructor) {
    ...
    for (const propertyName of Object.getOwnPropertyNames(constructor["prototype"])) {
      ...
      const descriptor = import_lib.Utils.getNonNullable(nullableDescriptor);
      const isMethod = descriptor.value instanceof Function;
      import_lib.Utils.assert(isMethod, "cannot add logging to properties that are not methods!");
...
//# sourceMappingURL=chunk-6Q2SZNEY.js.map
