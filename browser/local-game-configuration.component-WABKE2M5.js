...
var LocalGameConfigurationComponent = class _LocalGameConfigurationComponent extends BaseWrapperComponent {
  ...
  configDemo;
  rulesConfig = import_lib.MGPOptional.empty();
  setConfigDemo(config) {
    const stateProvider = this.getStateProvider();
    if (stateProvider.isPresent()) {
      const node = new GameNode(stateProvider.get()(import_lib.MGPOptional.of(config)));
      this.configDemo = {
        title: this.getGameName().get(),
        click: import_lib.MGPOptional.empty(),
  ...
  startGame() {
    return __async(this, null, function* () {
      import_lib.Utils.assert(this.rulesConfig.isPresent(), "Cannot start the game without having chosen a config");
      ...
      if ((0, import_lib.comparableEquals)(rulesConfig, defaultConfig)) {
        return this.router.navigate(["/local", this.getGameUrlName()]);
      } else {
...
//# sourceMappingURL=local-game-configuration.component-WABKE2M5.js.map
