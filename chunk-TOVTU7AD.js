...
var AbstractCurrentGameService = class _AbstractCurrentGameService {
  static roleToMessage = new import_lib.MGPMap([
  ...
  clearCurrentGame() {
    this.currentGame = import_lib.MGPOptional.empty();
    this.currentGameRS.next(import_lib.MGPOptional.empty());
  }
  ...
  canUserCreate() {
    if (this.currentGame.isAbsent()) {
      ...
      return import_lib.MGPValidation.failure(message);
    }
  }
...
var CurrentGameService = class _CurrentGameService extends AbstractCurrentGameService {
  ...
  onCurrentGameUpdate(newCurrentGame) {
    ...
    if (valueChanged) {
      this.changeCurrentGame(import_lib.MGPOptional.ofNullable(newCurrentGame));
    }
  }
  ngOnDestroy() {
    this.currentGameSubscription.unsubscribe();
...
//# sourceMappingURL=chunk-TOVTU7AD.js.map
