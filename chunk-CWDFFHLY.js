...
var AbstractBackendService = class {
  callbacks = new import_lib.MGPMap();
      ...
      return new Promise((resolve) => {
        const onMessage = (message) => {
          ...
          if (message.tag === "Error") {
            resolve(import_lib.MGPFallible.failure(message.getArgument("reason")));
          } else {
            resolve(import_lib.MGPFallible.success(message));
          }
        };
...
var BackendService = class _BackendService extends AbstractBackendService {
  ...
  webSocket = import_lib.MGPOptional.empty();
  connectionPromise;
  resolveConnection;
  ...
  connect() {
    return __async(this, null, function* () {
      import_lib.Utils.assert(this.webSocket.isAbsent(), "Should not connect twice to WebSocket!");
      ...
      return new Promise((resolve) => {
        ...
        let timeout = import_lib.MGPOptional.empty();
        const reconnect = () => {
          ...
          timeout = import_lib.MGPOptional.of(setTimeout(() => __async(this, null, function* () {
        ...
        ws.onopen = () => {
          if (timeout.isPresent()) {
            ...
            timeout = import_lib.MGPOptional.empty();
          }
          this.webSocket = import_lib.MGPOptional.of(ws);
        ...
        ws.onerror = (_error) => {
          ...
          this.webSocket = import_lib.MGPOptional.empty();
          reconnect();
        };
        ws.onclose = () => {
          this.webSocket = import_lib.MGPOptional.empty();
        ...
        ws.onmessage = (ev) => {
          import_lib.Utils.assert(typeof ev.data === "string", `Received malformed WebSocket message (not a string): ${JSON.stringify(ev.data)}`);
          const json = import_lib.Utils.getNonNullable(JSON.parse(ev.data));
          import_lib.Utils.assert(
            typeof json === "object",
            // i.e., an array
          ...
          const tag = json[0];
          const args = json[1];
          import_lib.Utils.assert(tag != null && typeof tag === "string", `Received malformed WebSocket message (missing tag): ${JSON.stringify(json)}`);
  ...
  disconnect() {
    import_lib.Utils.assert(this.webSocket.isPresent(), "Should not disconnect from unconnected WebSocket!");
...
//# sourceMappingURL=chunk-CWDFFHLY.js.map
