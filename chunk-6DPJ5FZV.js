...
// src/app/dao/FirestoreDAO.ts
var import_lib = __toESM(require_dist());
var FirestoreDAO = class {
  collectionName;
  collection;
  constructor(collectionName) {
    ...
    this.collection = collection(firestore, this.collectionName).withConverter(genericConverter);
  }
  create(newElement) {
    return __async(this, null, function* () {
      const docRef = yield addDoc(this.collection, newElement);
      return docRef.id;
    });
  }
  read(id) {
    return __async(this, null, function* () {
      const docSnapshot = yield getDoc(doc(this.collection, id));
      if (docSnapshot.exists()) {
        return import_lib.MGPOptional.of(import_lib.Utils.getNonNullable(docSnapshot.data()));
      } else {
        return import_lib.MGPOptional.empty();
      }
    });
  }
  exists(id) {
    return __async(this, null, function* () {
      return (yield this.read(id)).isPresent();
    });
  }
  update(id, update) {
    return __async(this, null, function* () {
  ...
  subscribeToChanges(id, callback) {
    return new Subscription(onSnapshot(doc(this.collection, id), (doc2) => {
      callback(import_lib.MGPOptional.ofNullable(doc2.data()));
    }));
  }
  findWhere(conditions) {
    return __async(this, null, function* () {
      const query2 = this.constructQuery(conditions);
      const snapshot = yield getDocs(query2);
  ...
  constructQuery(conditions) {
    let query2 = query(this.collection);
    for (const condition of conditions) {
      query2 = query(query2, where(condition[0], condition[1], condition[2]));
    }
    return query2;
  }
};

// src/app/dao/UserDAO.ts
...
var UserDAO = class UserDAO2 extends FirestoreDAO {
  constructor() {
    super("users");
  }
  static \u0275fac = function UserDAO_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || UserDAO2)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: UserDAO2, factory: UserDAO2.\u0275fac, providedIn: "root" });
};
UserDAO = __decorate([
  Debug.log
], UserDAO);
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(UserDAO, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
var UserService = class _UserService {
  ...
  usernameIsAvailable(username) {
    return __async(this, null, function* () {
      const usersWithSameUsername = yield this.userDAO.findWhere([["username", "==", username]]);
      return usersWithSameUsername.length === 0;
    });
  }
  setUsername(uid, username) {
    return __async(this, null, function* () {
      yield this.userDAO.update(uid, { username });
    });
  }
  markAsVerified(uid) {
    return __async(this, null, function* () {
      yield this.userDAO.update(uid, { verified: true });
    });
  }
...
function _performApiRequest(_0, _1, _2, _3) {
  return __async(this, arguments, function* (auth, method, path, request, customErrorMap = {}) {
    return _performFetchWithErrorHandling(auth, customErrorMap, () => __async(null, null, function* () {
      ...
      const query2 = querystring(__spreadValues({
      ...
      return FetchProvider.fetch()(yield _getFinalTarget(auth, auth.config.apiHost, path, query2), fetchArgs);
    }));
  });
}
...
// src/app/services/ConnectedUserService.ts
...
var Auth = class {
  static createUserWithEmailAndPassword(auth, email, password) {
    return createUserWithEmailAndPassword(auth, email, password);
  }
  static sendEmailVerification(user) {
    return sendEmailVerification(user);
  }
  static sendPasswordResetEmail(auth, email) {
    return sendPasswordResetEmail(auth, email);
  }
  static signInWithEmailAndPassword(auth, email, password) {
    return __async(this, null, function* () {
      const credential = yield signInWithEmailAndPassword(auth, email, password);
      return credential.user;
    });
  }
  static signInWithPopup(auth, provider) {
    return __async(this, null, function* () {
...
var AuthUser = class _AuthUser {
   ...
   */
  static NOT_CONNECTED = new _AuthUser("", import_lib2.MGPOptional.empty(), import_lib2.MGPOptional.empty(), false);
  /**
   ...
   */
  constructor(id, email, username, verified) {
    this.id = id;
    this.email = email;
  ...
  equals(other) {
    return this.id === other.id && this.email.equals(other.email) && this.username.equals(other.username) && this.verified === other.verified;
  }
};
var ConnectedUserService = class ConnectedUserService2 {
  userDAO = inject(UserDAO);
   ...
   */
  user = import_lib2.MGPOptional.empty();
  userRS;
  userObs;
  ...
  constructor() {
    this.auth = getAuth();
    this.userRS = new ReplaySubject(1);
    this.userObs = this.userRS.asObservable();
    this.authSubscription = new Subscription(onAuthStateChanged(this.auth, (user) => __async(this, null, function* () {
      if (user == null) {
        Debug.display("ConnectedUserService", "subscription", "User is not connected");
        this.userSubscription.unsubscribe();
        this.userRS.next(AuthUser.NOT_CONNECTED);
        this.user = import_lib2.MGPOptional.empty();
      } else {
        import_lib2.Utils.assert(this.user.isAbsent(), "ConnectedUserService received a double update for an user, this is unexpected");
        this.userSubscription = this.userDAO.subscribeToChanges(user.uid, (docOpt) => {
          if (docOpt.isPresent()) {
            const doc2 = docOpt.get();
            const username = doc2.username;
            ...
            const userHasFinalizedVerification = this.emailVerified(user) && username != null;
            if (userHasFinalizedVerification && doc2.verified === false) {
              return this.userService.markAsVerified(user.uid);
            }
            const authUser = new AuthUser(user.uid, import_lib2.MGPOptional.ofNullable(user.email), import_lib2.MGPOptional.ofNullable(username), userHasFinalizedVerification);
            if (this.user.equalsValue(authUser) === false) {
              this.user = import_lib2.MGPOptional.of(authUser);
  ...
  sendPasswordResetEmail(email) {
    return __async(this, null, function* () {
      try {
        yield Auth.sendPasswordResetEmail(this.auth, email);
        return import_lib2.MGPValidation.SUCCESS;
      } catch (e) {
  ...
  doRegister(username, email, password) {
    return __async(this, null, function* () {
      if (yield this.userService.usernameIsAvailable(username)) {
        return this.registerAfterUsernameCheck(username, email, password);
      } else {
        return import_lib2.MGPFallible.failure($localize`This username is already in use.`);
      }
    });
  }
  registerAfterUsernameCheck(username, email, password) {
    return __async(this, null, function* () {
      try {
        const userCredential = yield Auth.createUserWithEmailAndPassword(this.auth, email, password);
        yield Auth.signInWithEmailAndPassword(this.auth, email, password);
        const user = import_lib2.Utils.getNonNullable(userCredential.user);
        yield this.createUser(user.uid, username);
        return import_lib2.MGPFallible.success(user);
      } catch (e) {
        return this.catchFirebaseError(e);
      }
    });
  }
  catchFirebaseError(e) {
    if (e instanceof FirebaseError) {
      return import_lib2.MGPFallible.failure(this.mapFirebaseError(e));
    } else {
      throw e;
    }
  }
  mapFirebaseError(error) {
    switch (error.code) {
      ...
      default:
        import_lib2.Utils.logError("ConnectedUserService", "Unsupported firebase error", { errorCode: error.code, errorMessage: error.message });
        return error.message;
    }
  }
  sendEmailVerification() {
    return __async(this, null, function* () {
      const user = import_lib2.MGPOptional.ofNullable(this.auth.currentUser);
      if (user.isPresent()) {
        if (this.emailVerified(user.get())) {
          return import_lib2.Utils.logError("ConnectedUserService", "Verified users should not ask email verification after being verified");
        }
        try {
          yield Auth.sendEmailVerification(user.get());
          return import_lib2.MGPValidation.SUCCESS;
        } catch (e) {
          return this.catchFirebaseError(e);
        }
      } else {
        return import_lib2.Utils.logError("ConnectedUserService", "Unlogged users cannot request for email verification");
      }
    });
  }
  /**
  ...
  createUser(uid, username) {
    return __async(this, null, function* () {
      if (username == null) {
        yield this.userDAO.set(uid, { verified: false });
      } else {
        yield this.userDAO.set(uid, { username, verified: false });
      }
    });
  }
  doGoogleLogin() {
    return __async(this, null, function* () {
      return import_lib2.MGPValidation.ofFallible(yield this.registerOrLoginWithGoogle());
    });
  }
  registerOrLoginWithGoogle() {
    return __async(this, null, function* () {
      try {
        ...
        const user = yield Auth.signInWithPopup(this.auth, provider);
        if ((yield this.userDAO.exists(user.uid)) === false) {
          yield this.createUser(user.uid);
        }
        return import_lib2.MGPFallible.success(user);
      } catch (e) {
        return this.catchFirebaseError(e);
  ...
  disconnect() {
    return __async(this, null, function* () {
      ...
      if (user.isPresent()) {
        ...
        return import_lib2.MGPValidation.failure("Cannot disconnect a non-connected user");
      }
    });
  }
  ...
  setUsername(username) {
    return __async(this, null, function* () {
      if (username === "") {
        return import_lib2.MGPValidation.failure($localize`Your username may not be empty.`);
      }
      try {
        const available = yield this.userService.usernameIsAvailable(username);
        if (available === false) {
          return import_lib2.MGPValidation.failure($localize`This username is already in use, please select a different one.`);
        }
        const currentUser = import_lib2.Utils.getNonNullable(this.auth.currentUser);
        yield Auth.updateProfile(currentUser, { displayName: username });
        yield this.userService.setUsername(currentUser.uid, username);
        yield this.userService.markAsVerified(currentUser.uid);
        yield this.reloadUser();
        return import_lib2.MGPValidation.SUCCESS;
      } catch (e) {
        return this.catchFirebaseError(e);
      }
  ...
  setPicture(url) {
    return __async(this, null, function* () {
      try {
        const currentUser = import_lib2.Utils.getNonNullable(this.auth.currentUser);
        yield Auth.updateProfile(currentUser, { photoURL: url });
        return import_lib2.MGPValidation.SUCCESS;
      } catch (e) {
        return this.catchFirebaseError(e);
      }
  ...
  reloadUser() {
    return __async(this, null, function* () {
      const currentUser = import_lib2.Utils.getNonNullable(this.auth.currentUser);
      yield currentUser.getIdToken(true);
  ...
  static \u0275fac = function ConnectedUserService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || ConnectedUserService2)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: ConnectedUserService2, factory: ConnectedUserService2.\u0275fac, providedIn: "root" });
};
ConnectedUserService = __decorate2([
  Debug.log
], ConnectedUserService);
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(ConnectedUserService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
@firebase/auth/dist/esm/index-dfb5c973.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-6DPJ5FZV.js.map
